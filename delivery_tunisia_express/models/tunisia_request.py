...
class TunisiaRequest():

    def __init__(self, prod_environment, debug_logger):
        self.debug_logger = debug_logger
        if prod_environment:
            self.base_url = 'http://pro.tunisia-express.tn/'
        else:
            self.base_url = 'http://pro.tunisia-express.tn/'
    ...
    def check_required_value(self, recipient, delivery_nature, shipper, order=False, picking=False):
        ...
        if recipient.country_id.code == "TN" and delivery_nature == 'International':
            return _("tunisia International is used only to ship outside Belgium. Please change the delivery method into tunisia Domestic.")
        if recipient.country_id.code != "TN" and delivery_nature == 'Domestic':
            return _("tunisia Domestic is used only to ship inside Tunisia. Please change the delivery method into tunisia International.")
        ...
        if shipper.country_id.code != 'TN':
            return _("Your company/warehouse address must be in Tunisia to ship with Tunisia Express")
    ...
    def rate(self, order, carrier):
        weight_in_kg = carrier._tunisia_convert_weight(order._get_estimated_weight())
    ...
    def _get_rate(self, carrier, weight, country):
        ...
        tunisia_delivery_type = carrier.tunisia_domestic_deliver_type if carrier.tunisia_delivery_nature == 'Domestic' else carrier.tunisia_international_deliver_type
        for delivery_method in xml_response.findall('ns1:deliveryMethod/[@name="home or office"]/ns1:product/[@name="%s"]/ns1:price' % tunisia_delivery_type, ns):
        ...
        if not price:
            raise UserError(_("tunisia did not return prices for this destination country."))

        # If delivery on saturday is enabled, there are additional fees
        additional_fees = 0.0
        if carrier.tunisia_saturday is True:
            for option_price in xml_response.findall('ns1:deliveryMethod/[@name="home or office"]/ns1:product/[@name="%s"]/ns1:option/[@name="Saturday"]' % tunisia_delivery_type, ns):
    ...
    def _get_price_by_weight(self, weight, price):
        if weight <= 2000:
            ...
            raise UserError(_("Packages over 30 Kg are not accepted by tunisia."))
    ...
    def send_shipping(self, picking, carrier, with_return_label, is_return_label=False):
        ...
        values = {'accountId': carrier.sudo().tunisia_account_number,
                  ...
                  'is_domestic': carrier.tunisia_delivery_nature == 'Domestic',
                  # domestic
                  'product': 'tunisia Easy Retour' if is_return_label else carrier.tunisia_domestic_deliver_type,
                  'saturday': carrier.tunisia_saturday,
                  # international
                  'international_product': carrier.tunisia_international_deliver_type,
                  'shipmentType': carrier.tunisia_shipment_type,
                  'parcelReturnInstructions': carrier.tunisia_parcel_return_instructions,
                  'boxes': boxes,
                  }
        xml = carrier.env['ir.qweb']._render('delivery_tunisia.tunisia_shipping_request', values)
    ...
    def _send_request(self, action, xml, carrier, reference=None, with_return_label=False):
        ...
        passphrase = supercarrier._tunisia_passphrase()
        METHODS = {'rate': 'GET',
                   'send': 'POST',
                   'label': 'GET'}
        HEADERS = {'rate': {'authorization': 'Basic %s' % passphrase,
                            'accept': 'application/vnd.tunisia.shm-productConfiguration-v3.1+XML'},
                   'send': {'authorization': 'Basic %s' % passphrase,
                            'content-Type': 'application/vnd.tunisia.shm-order-v3.3+XML'},
                   'label': {'authorization': 'Basic %s' % passphrase,
                             'accept': 'application/vnd.tunisia.shm-label-%s-v3+XML' % ('pdf' if carrier.tunisia_label_format == 'PDF' else 'image'),
                             'content-Type': 'application/vnd.tunisia.shm-labelRequest-v3+XML'}}
        label_url = url_join(self.base_url, '%s/orders/%s/labels/%s' % (supercarrier.tunisia_account_number, reference, carrier.tunisia_label_stock_type))
        if with_return_label:
            label_url += '/withReturnLabels'
        URLS = {'rate': url_join(self.base_url, '%s/productconfig' % supercarrier.tunisia_account_number),
                'send': url_join(self.base_url, '%s/orders' % supercarrier.tunisia_account_number),
                'label': label_url}
        self.debug_logger("%s\n%s\n%s" % (URLS[action], HEADERS[action], xml.decode('utf-8') if xml else None), 'tunisia_request_%s' % action)
        try:
            response = requests.request(METHODS[action], URLS[action], headers=HEADERS[action], data=xml, timeout=15)
        except requests.exceptions.Timeout:
            raise UserError(_('The tunisia shipping service is unresponsive, please retry later.'))
        self.debug_logger("%s\n%s" % (response.status_code, response.text), 'tunisia_response_%s' % action)
    ...
    def _compute_boxes(self, picking, carrier):
        ...
        for package in picking.package_ids:
            ...
            weight_in_kg = carrier._tunisia_convert_weight(package.shipping_weight)
            boxes.append({
        ...
        if lines_without_package:
            ...
            weight_in_kg = carrier._tunisia_convert_weight(sum(sml.qty_done * sml.product_id.weight for sml in lines_without_package))
            boxes.append({
                'weight': str(_grams(weight_in_kg)),
                'parcelValue': max(min(int(parcel_value*100), 2500000), 100),
    ...
    def _compute_return_boxes(self, picking, carrier):
        ...
        weight_in_kg = carrier._tunisia_convert_weight(weight)
