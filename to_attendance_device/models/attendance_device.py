...
class AttendanceDevice(models.Model):
    ...
    def _get_default_attendance_states(self):
        ...
        attendance_device_state_code_0 = self.env.ref(
            'to_attendance_device.attendance_device_state_code_0')
        if attendance_device_state_code_0:
            att_state_ids += attendance_device_state_code_0
        attendance_device_state_code_1 = self.env.ref(
            'to_attendance_device.attendance_device_state_code_1')
        if attendance_device_state_code_1:
            att_state_ids += attendance_device_state_code_1

        # Overtime Working Attendance
        attendance_device_state_code_4 = self.env.ref(
            'to_attendance_device.attendance_device_state_code_4')
        if attendance_device_state_code_4:
            att_state_ids += attendance_device_state_code_4
        attendance_device_state_code_5 = self.env.ref(
            'to_attendance_device.attendance_device_state_code_5')
    ...
    port = fields.Integer(string="Port", required=True, default=4370,
    ...
    device_users_count = fields.Integer(
        string='Users Count', compute='_compute_device_users_count', store=True)
    ...
    mapped_employees_count = fields.Integer(
        string='Mapped Employee Count', compute='_compute_mapped_employees_count', store=True)
    ...
    user_attendance_ids = fields.One2many(
        'user.attendance', 'device_id', string='Attendance Data', readonly=True)
    total_att_records = fields.Integer(
        string='Attendance Records', compute='_compute_total_attendance_records')
    finger_template_ids = fields.One2many(
        'finger.template', 'device_id', string='Finger Template', readonly=True)
    ...
    def _compute_device_users_count(self):
        total_att_data = self.env['attendance.device.user'].read_group(
            [('device_id', 'in', self.ids)], ['device_id'], ['device_id'])
        mapped_data = dict([(dict_data['device_id'][0], dict_data['device_id_count'])
                           for dict_data in total_att_data])
        for r in self:
            r.device_users_count = mapped_data.get(r.id, 0)

    def _compute_total_finger_template_records(self):
        total_att_data = self.env['finger.template'].read_group(
            [('device_id', 'in', self.ids)], ['device_id'], ['device_id'])
        mapped_data = dict([(dict_data['device_id'][0], dict_data['device_id_count'])
                           for dict_data in total_att_data])
        for r in self:
    ...
    def _compute_total_attendance_records(self):
        total_att_data = self.env['user.attendance'].read_group(
            [('device_id', 'in', self.ids)], ['device_id'], ['device_id'])
        mapped_data = dict([(dict_data['device_id'][0], dict_data['device_id_count'])
                           for dict_data in total_att_data])
        for r in self:
    ...
    def connect(self):
        ...
        def post_message():
            ...
            #     'to_attendance_device.email_template_attendance_device')
        ...
        try:
            ...
            error_msg = _("Could not reach the device %s (ping %s)") % (
                self.display_name, self.ip)
        except ZKConnectionUnauthorized as e:
            error_msg = _(
                "Connection Unauthorized! The device %s may require password") % self.display_name
    ...
    def clearData(self):
        ...
        try:
            ...
            raise ValidationError(
                _('Could not clear all data from the device %s') % (self.display_name,))
        finally:
            self.enableDevice()
            self.disconnect()
    ...
    def getAttendance(self):
        ...
        try:
            ...
            post_err_msg = True
            raise ValidationError(
                _('Could not get attendance data from the device %s') % (self.display_name,))

        finally:
            # if post_err_msg and self.download_error_notification:
            #     email_template_id = self.env.ref(
            #         'to_attendance_device.email_template_error_get_attendance')
            #     self.post_message(email_template_id)
            self.enableDevice()
            self.disconnect()
    ...
    def _download_users_by_uid(self):
        ...
        for r in self:
            ...
            users_not_in_device = device_user_ids.filtered(
                lambda user: user.uid not in existing_user_ids)
    ...
    def action_show_time(self):
        ...
        """
        self.ensure_one()
        raise ValidationError(_("The Machine time is %s") %
                              self.getMachineTime())
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                ...
                user_id = AttendanceUser.with_context(active_test=False).search([
                    ...
                    ('device_id', '=', r.id)], limit=1)
                if user_id:
                    utc_timestamp = r.convert_time_to_utc(
                        attendance.timestamp, r.tz)
    ...
    def action_finger_template_download(self):
        ...
        for r in self:
            ...
            for template in template_data:
                template_datas.append(
                    str(template.uid) + '_' + str(template.fid))

            existing_finger_template_ids = []
            finger_template_ids = FingerTemplate.search(
                [('device_id', '=', r.id)])
            for template in finger_template_ids.filtered(lambda tmp: (str(tmp.uid) + '_' + str(tmp.fid)) in template_datas):
                existing_finger_template_ids.append(
                    str(template.uid) + '_' + str(template.fid))
    ...
    def action_attendance_clear(self):
        ...
        for r in self:
            ...
            if attendance_clear_safe:
                ...
                error_msg += _(
                    "It was not safe to clear attendance data from the device %s.<br />") % (r.name,)
                ...
                _logger.warning(
                    "It was not safe to clear attendance data from the device %s" % r.name)
                # if r.auto_clear_attendance_error_notif:
                #     email_template_id = self.env.ref(
                #         'to_attendance_device.email_template_not_safe_to_clear_attendance')
    ...
    def action_check_connection(self):
        ...
        if self.connect():
            self.disconnect()
            raise UserError(
                _('Connect to the device %s successfully') % (self.display_name,))
    ...
    def action_view_users(self):
        ...
        if self.device_users_count != 1:
            result['domain'] = "[('id', 'in', " + \
                str(self.device_user_ids.ids) + ")]"
        elif self.device_users_count == 1:
            res = self.env.ref(
                'to_attendance_device.attendance_device_user_form_view', False)
    ...
    def action_view_attendance_data(self):
        self.ensure_one()
        action = self.env.ref(
            'to_attendance_device.action_user_attendance_data')
        result = action.read()[0]
        ...
        if total_att_records != 1:
            ...
            res = self.env.ref(
                'to_attendance_device.view_attendance_data_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_mapped_employees(self):
        ...
        if self.mapped_employees_count != 1:
            result['domain'] = "[('id', 'in', " + \
                str(self.mapped_employee_ids.ids) + ")]"
        elif self.mapped_employees_count == 1:
            res = self.env.ref(
                'to_attendance_device.view_employee_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_finger_template(self):
        ...
        if total_finger_template_records != 1:
            ...
            res = self.env.ref(
                'to_attendance_device.view_finger_template_form', False)
            result['views'] = [(res and res.id or False, 'form')]
...
class AttendanceDeviceStateLine(models.Model):
    ...
    attendance_state_id = fields.Many2one(
        'attendance.state', string='State Code', required=True, index=True,)
    device_id = fields.Many2one('attendance.device', string='Device',
                                required=True, ondelete='cascade', index=True, copy=False)
    code = fields.Integer(
        string='Code Number', related='attendance_state_id.code', store=True, readonly=True)
